-- ChainLoom
-- Final RAW refinement validation
-- Version: 1.3

USE DATABASE CHAINLOOM;
USE SCHEMA RAW;

-- 1. Production severity
SELECT
    product_id,
...
-- 4. Coverage gate: all four affected products must exist in both facts.
SELECT
    COUNT(DISTINCT ol.product_id) AS affected_order_products,
    COUNT(DISTINCT s.product_id) AS affected_shipment_products
FROM ORDER_LINE ol
LEFT JOIN SHIPMENT s
    ON ol.order_line_id = s.order_line_id
WHERE ol.plant_id = 'PL03'
