...
def render_priority_attention(df):
    """
    Render the highest-priority product risk signal.
    """
    ...
    with st.container(border=True):
        ...
        with left:
            st.caption(
                f"{row['PRODUCT_ID']} · HIGH ATTENTION"
            )
            ...
            st.write(
                f"**{int(row['RISK_SIGNAL_COUNT'])} independent "
        ...
        with right:
            st.metric(
                "Fulfillment",
                _format_percentage(row["FULFILLMENT_RATE"]),
            )

            st.metric(
                "On-Time Delivery",
                _format_percentage(row["ON_TIME_DELIVERY_RATE"]),
            )

        st.caption(
            "Investigation areas are independent observed signals; "
    ...
    display_df = pd.DataFrame(
        {
            ...
            "Risk Signals": df["RISK_SIGNAL_COUNT"]
            .fillna(0)
            .astype(int),
        }
    )
